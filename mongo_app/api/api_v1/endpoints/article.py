...
@router.post("/articles", response_model=ArticleInResponse, tags=["articles"], status_code=HTTP_201_CREATED)
...
@router.post("/articles/{slug}/favorite", response_model=ArticleInResponse, tags=["articles"])
...
@router.delete("/articles/{slug}/favorite", response_model=ArticleInResponse, tags=["articles"])
...
@router.post("/articles2", response_model=ArticleInResponse, tags=["articles"], status_code=HTTP_201_CREATED)
async def create_new(article: ArticleInCreate = Body(..., embed=True), db: AsyncIOMotorClient = Depends(get_database), ):
    
    dbarticle = await create_article_by_slug(db, article, "Pedro")
    return create_aliased_response(ArticleInResponse(article=dbarticle))